...
class RULEstimate:
    subsystem: str
    remaining_flight_hours: Optional[float]
    confidence: float
...
class RULEstimator:
    ...
    def estimate(self) -> List[RULEstimate]:
        ...
        for subsystem, (metric_key, threshold, direction) in self.SUBSYSTEM_METRICS.items():
            ...
            if len(hist) < 20:
                results.append(RULEstimate(subsystem, None, 0.0, 0.0, metric_key))
                continue
            ...
            results.append(RULEstimate(subsystem, None if remaining is None else round(remaining, 2), round(confidence, 2),
...
class DiagnosticEngine:
    ...
    def _compute_subsystem_health(self, feat: Dict[str, float], rul_list: List[RULEstimate]) -> Dict[str, float]:
        ...
        for subsystem in ("cooling", "fuel_injection", "lubrication"):
            ...
            if (r and r.remaining_flight_hours is not None
                    and r.remaining_flight_hours < 5.0 and r.confidence > 0.3):
